    ...
    internal class Level
    {
        ...
        public Level(int width, int height, HeroTile hero = null)
        {
            ...
            // If hero wasn't placed (because it was null), place it randomly
                ...
                if (_hero == null)
                {
                    _hero = CreateTile(TileType.Hero, randomPosition) as HeroTile;
                }
                else
                {
                    _hero.XCoordinate = randomPosition.XCoordinate;
                    _hero .YCoordinate = randomPosition.YCoordinate;
        ...
        private Position GetRandomEmptyPosition()
        {
            ...
            while (OpenPosition == false) 
            { 
                 x = random.Next(0, _width);
                y = random.Next(0, _height);

                if (_tiles[x, y].Display == '.')
                {
